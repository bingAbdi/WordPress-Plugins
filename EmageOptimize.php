...
function emage_serve_retina_images($image, $attachment_id, $size) {
    $emage_option = get_option('emage_option');
    if ($emage_option === 'maximum' && function_exists('wp_get_attachment_image_src')) {
        ...
        if ($retina_image && isset($retina_image[0])) {
            $image[0] = $retina_image[0];
            $image[1] = $retina_image[1];
            $image[2] = $retina_image[2];
        }
    }
    return $image;
}
...
function emage_render_settings_page() {
    ...
    if (isset($_POST['emage_save_settings'])) {
        ...
        $emage_option = isset($_POST['emage_option']) ? $_POST['emage_option'] : '';
        update_option('emage_option', $emage_option);
        $emage_format = isset($_POST['emage_format']) ? sanitize_text

// Serve high-rfunction emage_override_image_sizes($sizes) {
    $emage_option = get_option('emage_option');
    if ($emage_option === 'maximum') {
        $sizes['large'] = array(
            'width' => get_option('large_size_w') * 2,
...
function emage_serve_retina_images($image_url, $attachment_id, $size) {
    $emage_option = get_option('emage_option');
    if ($emage_option === 'maximum' && function_existsesolution images for retina displays
function emage_serve_retina_images($image_url, $attachment_id, $size) {
    $emage_option = get_option('emage_option');
    if ($emage_option === 'maximum' && function_exists('wp_get_attachment_image_src')) {
        $retina_image = wp_get_attachment_image_src($attachment_id, $size . '_retina');
